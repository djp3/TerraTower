...
public class HandlerBuildTower extends HandlerAbstractLocation {
	...
	public HandlerBuildTower(TTEventWrapperQueuer eventPublisher) {
		super(eventPublisher);
	}

	@Override
	public HandlerAbstract copy() {
		return new HandlerBuildTower(this.getEventPublisher());
	}
	...
	public Pair<byte[], byte[]> handle(InetAddress ip, HTTPRequest httpRequestType, Map<String, String> headers, String restFunction, Map<String, String> parameters) {
		...
		if(errors.size() != 0){
			...
			TTEventBuildTower tt = new TTEventBuildTower(getWorldName(),getWorldPassword(),getPlayerName(),getPlayerPassword(),getLat(),getLng(),getAlt());
